...
class InvoiceService:
    ...
    def create_invoice_for_user(self, user_id, items_data, due_date=None):
        ...
        RepoRegistry.get(RepoName.ITEM).bulk_insert(items_data)
        ...
        RepoRegistry.get(RepoName.INVOICE).insert(invoice)

        return invoice

    def get_all_invoices(self):
        """Get all invoices."""
        return RepoRegistry.get(RepoName.INVOICE).get_all()

    def get_invoices_for_user(self, user_id):
        """Get all invoices for a specific user."""
        return [inv for inv in RepoRegistry.get(RepoName.INVOICE).get_by_user_id(user_id)]
